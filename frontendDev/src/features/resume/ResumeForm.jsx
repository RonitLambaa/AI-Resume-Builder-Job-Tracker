...
import { createResume } from "./resumeSlice";
        ...
        <div>
          ...
          <div className="space-y-4">
            {educationFields.fields.map((item, index) => (
              <div key={item.id} className="p-4 rounded-lg border bg-gray-50 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Input label="Start Date" type="date" {...register(`sections.education.${index}.startDate`)} />
                  <Input label="End Date" type="date" {...register(`sections.education.${index}.endDate`)} />
                </div>
                <button
                  type="button"
            ...
            <button
              type="button"
              onClick={() => educationFields.append({ school: "", degree: "", startDate: "", endDate: "" })}
        ...
        <div>
          ...
          <div className="space-y-4">
            {experienceFields.fields.map((item, index) => (
              <div key={item.id} className="p-4 rounded-lg border bg-gray-50 space-y-3">
                <Input label="Company" {...register(`sections.experience.${index}.company`)} />
                <Input label="Position" {...register(`sections.experience.${index}.position`)} />
                <div className="grid grid-cols-2 gap-4">
                  <Input label="Start Date" type="date" {...register(`sections.experience.${index}.startDate`)} />
                  <Input label="End Date" type="date" {...register(`sections.experience.${index}.endDate`)} />
                </div>
                <Textarea label="Description" {...register(`sections.experience.${index}.description`)} />
                <button
                  type="button"
          ...
          <div className="space-y-2">
            {skillsFields.fields.map((item, index) => (
              <div key={item.id} className="flex gap-2 items-center">
                <Input placeholder="Skill" {...register(`sections.skills.${index}`)} />
                <button
                  type="button"
        ...
        <div>
          ...
          <div className="space-y-4">
            {projectsFields.fields.map((item, index) => (
              <div key={item.id} className="p-4 rounded-lg border bg-gray-50 space-y-3">
                <Input label="Name" {...register(`sections.projects.${index}.name`)} />
                <Textarea label="Description" {...register(`sections.projects.${index}.description`)} />
                <Input label="Link" {...register(`sections.projects.${index}.link`)} />
                <button
                  type="button"
            ...
            <button
              type="button"
              onClick={() => projectsFields.append({ name: "", description: "", link: "" })}
              className="px-4 py-2 rounded-lg bg-emerald-700 text-white hover:bg-emerald-600 transition cursor-pointer flex justify-between items-center gap-1"
            >
             <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="white" class="bi bi-plus" viewBox="0 0 16 16">
                <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4"/>
